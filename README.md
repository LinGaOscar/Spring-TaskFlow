...
企業後台 **Kanban 任務看板多人即時協作系統**。以部／科兩層組織為基礎，讓各科團隊在看板上建立、指派、拖曳任務並即時協作，同時以部門隔離確保跨科資料不外洩。

> 技術架構、開發與部署方式見 [`docs/dev.md`](docs/dev.md)。

## 系統目標

- 以 **Kanban 三欄（待辦 / 進行中 / 完成）** 管理團隊任務，拖曳即改變狀態
- **多人即時協作**：任一成員的異動即時同步給看板上其他在線成員，無需重整
- **部門隔離**：組織分部（Division）→ 科（Section）兩層，各科看板彼此不可見，部長可跨科唯讀查閱
- **完成即歸檔**：看板完成後歸檔為歷史唯讀，可查詢與還原
- **任務模板**：以科為單位共用任務模板，加速常見任務建立
...
- 歸檔的看板對全員強制唯讀
- 部門隔離貫穿頁面、REST、WebSocket 訂閱與匯出四個入口
...
| Session | 伺服器端 Session（HttpOnly Cookie，無 JWT） |
...
| 拖曳任務 | 跨欄拖曳改變狀態、欄內拖曳調整排序，重整後狀態保留 |
...
| 權限保護 | 每次寫入操作皆驗證寫入權限；訂閱看板時驗證讀取權限 |
...
|---|---|
| 查看成員 | 列出本科每個看板的成員清單 |
| 新增成員 | 將科內帳號加入指定看板（部長自建看板可加入下屬科成員） |
